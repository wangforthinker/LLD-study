...
{	
	...
	if(!first)
	{
		...
		printk("in scull_follow ,create first\n");
		first->data = NULL;
		first->next = NULL;
		dev->data = first;	
	}
...
ssize_t scull_write(struct file* filp , const char __user* data , size_t count , loff_t* f_pos)
{
	...
	if(!dptr->data[s_pos])
	{
		dptr->data[s_pos] = (void*)__get_free_pages(GFP_KERNEL , 0);
		if(!dptr->data[s_pos])
			goto out;
...
void scull_vma_open(struct vm_area_struct* vma)
{
	...
	dev->mmap_count ++;
	printk("In scull_vma_open , mmap_count is %d\n", dev->mmap_count);
}
...
void scull_vma_close(struct vm_area_struct* vma)
{
	struct scull_dev* dev = vma->vm_private_data;
	dev->mmap_count --;
	printk("In scull_vma_close , mmap_count is %d\n", dev->mmap_count);
}

int scull_vma_fault(struct vm_area_struct* vma , struct vm_fault* vm_ft)
{
	struct page* retp = NULL;
	unsigned long offset;
	struct scull_dev* dev = vma->vm_private_data;
	void* ptr = NULL;
	pgoff_t pgoff = vm_ft->pgoff;	
	printk("in scull_vma_fault , pgoff is %ld , vma->vm_pgoff is %ld\n" ,pgoff , vma->vm_pgoff );
	...
	if(offset >= dev->size)
	{
		printk("In scull_vma_fault , address is out of memory \n");
		goto out;
	}
	
	offset >>= PAGE_SHIFT;
	struct scull_qset* qset = scull_follow(dev , offset/(unsigned long)(dev->qset));
	
	if(qset && qset->data)
		ptr = qset->data[offset%(unsigned long)(dev->qset)];
	
	
	if(!ptr)
		goto out;
	
	printk("In scull_vma_fault ,ptr is %ld\n",ptr);
	
	retp = virt_to_page(ptr);
